...
Not surprisingly, we can also round numbers in `R`. The main function to round numeric vectors numbers is called `round()` and takes a numeric vector or values as well as the **number of decimal places** to round to as input: 
...
As you can see, you have to bit careful: If you are asked to report to two meaningful numbers, some people may be inclined to use `round(189.48615, 2)`. However, as  189.48615 has more than two digits *before the decimal sign*, we do not need to report decimal places so that `round(189.48615, 0)` is sufficient for our purpose.


Let's give it a try yourself! 
...
question("Please round 89.48615 to two meaningful digits:",
  answer("89", correct = T, message = "As the number has two integer digits 'before' the decimal, you were not required to report digits 'after' the decimal. Thus, using round(89.48615, 0) is doing the job for you."),

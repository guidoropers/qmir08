...
One main exception are numbers with more than two digits *before the decimal sign* (for example, 1564.5464). In that case, please provide the whole number without digits after the decimal (unless you are specifically asked to). 
...
`R` provides us with two main functions to round numeric vectors numbers:

- `round(x, digits = 0)`: Rounds the values in its first argument (i.e., x) to the specified number of decimal places 
- `signif(x, digits = 0)`: Rounds the values in its first argument  (i.e., x) to the specified number of significant (= non-zero) digits

It might become more clear if you look at an example number like $189.48615$:
...
Hopefully, you see the differences. In most of your cases, the `round()` function is the one we need for our purposes. Let's give it a try! 
...
question("Please round 89.48615 to two meaningful digits:",
  ...
  answer("89.49", correct = F, message = "Too detailed as you provided two digits after the decimal."),
...
**Note:** As mentioned before, you may sometimes be specifically asked to round to a specified number of digits *after the decimal*. For example, if you are asked to round 125.10243214 to two (integer) digits after the decimal, please provide 125.10 in the solution.

**Note:** **Percentage values** are often stored as values that are bound between 0 and 1 (instead of 0% and 100%). In these cases, you will generally be required to report estimates in **percent**. 
